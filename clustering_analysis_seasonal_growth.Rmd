...
sbgmplot <- function(par,t,ylim,col="black",main="",xlab="",ylab="",type="l"){
  ...
  a1 <- exp(par[3])
  theta <- par[4]
  plot(t,sbgm(lmax,lmin,a1,theta,t=t),ylim=ylim,col=col,main=main,xlab=xlab,ylab=ylab,type=type, font.lab=1, cex.lab=1.5, cex.axis=1, lwd=4)
}
...
# plot
for(i in num_lis) {
  sbgmplot(estpara2[i,],t,c(0,max(data$fl)),col=cb[i])
  points(fl~days_from_hatching, data=data[data$ocluster_rename==i,],
         bg=cb[i], pch=21, cex=0.8, lwd=0.5)
}
...
# Plot growth trajectries of best cluster
