---
title: "clustering analysis - seasonal growth pattern in 2016-cohort"
author: "Shohei Noda"
date: "2025-02-15"
output: 
  html_document:
...
data: 2015 and 2016 cohorts  

```{r}

# remove files
file.remove("./input_data/id_list.csv")
file.remove("./input_data/rawdata_filtered.csv")
# copy input file
...
data <- data %>%
  filter(cohort == 2015 | cohort == 2016,
         ...
         days_from_hatching < 1500)
count(data, cohort, treatment)
...
for(i in num_lis) {
  ...
  growth_curve <- tibble(
    ...
    fl = sbgm(exp(estpara2[i,1]), 
              ...
              estpara2[i,4], 
              t=t)) %>% 
    filter(t >= min(data$days_from_hatching))
  # plot
  g <- g +
...
# Check AIC and BIC 
AIC：パラメータの数に応じて一定のペナルティを課す。  BIC：パラメータの数とサンプルサイズに応じてペナルティを課す。   
→ BICは大規模データセットにおいて過剰適合（オーバーフィッティング）を回避し、よりシンプルなモデルを選びやすい特徴がある。  
今回のサンプルサイズは大きいため、BICの方が適しているかもしれない。
...
ggplot(NULL) +
  # AIC
  geom_line(data = aic_bic, aes(x=clu, y=aic), linewidth = 2, color = "gray66") +
  geom_point(data = aic_bic, aes(x=clu, y=aic), size = 5, color = "gray66") + 
  # BIC
  geom_line(data = aic_bic, aes(x=clu, y=bic), linewidth = 2) +
  geom_point(data = aic_bic, aes(x=clu, y=bic), size = 5) + 
  labs(x = "Number of cluster", 
       y = "Value",
       caption = "AIC: gray, BIC: black")
...
# read data
d0 <- read.csv("./input_data/rawdata_filtered.csv")
d_gc <- read.csv("./output/k4_data.csv")

# filtering
d1 <- d0 %>% 
  left_join(d_gc, by="id") %>%
  filter(ocluster_rename %in% c(1,2,3,4)) %>% 
  mutate(growth = as.character(ocluster_rename))

# plot
color_list = brewer.pal(4, "RdYlBu") # color list
color_list = c("#D7191C","#FDAE61","#ABD9E9","#2C7BB6")
